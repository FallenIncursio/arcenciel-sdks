...
| [**resolveUsers**](UsersApi.md#resolveusersoperation) | **POST** /api/users/resolve | Resolve usernames to user IDs |
...
> { [key: string]: number | null; } resolveUsers(resolveUsersRequest)
...
import type { ResolveUsersOperationRequest } from '@arcenciel/sdk';
...
async function example() {
  ...
  const body = {
    // ResolveUsersRequest | Request payload for this operation.
    resolveUsersRequest: {"names":["value"]},
  } satisfies ResolveUsersOperationRequest;

  try {
    const data = await api.resolveUsers(body);
    console.log(data);
  } catch (error) {
    console.error(error);
  }
...
| Name | Type | Description  | Notes |
|------------- | ------------- | ------------- | -------------|
| **resolveUsersRequest** | [ResolveUsersRequest](ResolveUsersRequest.md) | Request payload for this operation. | |
