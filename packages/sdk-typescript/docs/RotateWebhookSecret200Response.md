
# RotateWebhookSecret200Response

200 response for api webhooks endpoints endpoint Id rotate secret used by the Arc en Ciel API contract.

## Properties

Name | Type
------------ | -------------
`data` | [DeveloperWebhookEndpoint](DeveloperWebhookEndpoint.md)
`previousSecretExpiresAt` | Date
`secret` | string

## Example

```typescript
import type { RotateWebhookSecret200Response } from '@arcenciel/sdk'

// TODO: Update the object below with actual values
const example = {
  "data": null,
  "previousSecretExpiresAt": null,
  "secret": null,
} satisfies RotateWebhookSecret200Response

console.log(example)

// Convert the instance to a JSON string
const exampleJSON: string = JSON.stringify(example)
console.log(exampleJSON)

// Parse the JSON string back to an object
const exampleParsed = JSON.parse(exampleJSON) as RotateWebhookSecret200Response
console.log(exampleParsed)
```

[[Back to top]](#) [[Back to API list]](../README.md#api-endpoints) [[Back to Model list]](../README.md#models) [[Back to README]](../README.md)
